    ...
    <div className="w-[60%] xl:w-[80%] mx-auto md:w-full md:px-10 sm:px-5">
      ...
      {loading && skills.length === 0 ? (
        ...
        <motion.div
          ...
          whileInView="visible"
          variants={variant}
          className="w-full mt-10 grid grid-cols-3 place-content-center sm:grid-cols-1 lg:grid-cols-2 gap-3"
        >
          {skills.length > 0 &&
            skills.map((item: ISkillsResponse, idx: number) => (
              <div
                className="w-full bg-gray-100 flex flex-col align-self-center items-center rounded-md py-4 "
                key={idx}
              >
                <Image
                  ...
                  alt={item.title}
                ></Image>
                <span className="text-gray-800 font-bold text-[15px] mt-2">
                  {item.title}
                </span>
